...
trait ResourceHelpers
{
        ...
        if (isset($this->files)) {
            foreach ($this->files as $file) {
                if (request()->hasFile($file) and request()->$file) {
                    ...
                    $data[$file] = $filePath;
                    Storage::disk(config('joda-resources.disk' ))->put($filePath, file_get_contents(request()->$file));
                }
            }
        }
        return $data;
    ...
    protected function deleteFilesIfExist($model)
    {
        if (isset($this->files)) {
            foreach ($this->files as $file) {
                if (Storage::disk(config('joda-resources.disk' ))->exists($model->$file)) {
                    Storage::disk(config('joda-resources.disk' ))->delete($model->$file);
                }
            }
        }
    }
